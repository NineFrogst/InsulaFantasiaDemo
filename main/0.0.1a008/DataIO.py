...
TODO: 优化程序：把主要的打印输出内容转移到对应的前端文件中，优化一些程序处理逻辑。*done*
'''
...
class Character:
    ...
    内置角色数目统计（但是暂时还没派上用场）
    通过输入角色的ID（cID）、名字（cName）、信息（cInfo）、属性（cPpty）作为参数来实例化类。
...
class Card:
    ...
    内置卡牌数目统计（但是暂时还没派上用场）
    通过输入卡牌的ID（kID）、名称（kName）、信息（kInfo）、属性（kPpty）作为参数来实例化类。
    ...
    def __init__(self,kID,kName,kInfo,kPpty):
        ...
        if   kID[0] == 'a' :
            ...
            Card.__countSUP += 1
        # try:
        #     self.handler = Cards.cardHandler(self.kID)
        # except:
        #     self.handler = None
...
def getIFDB(caseof,mode='get',lookID=None):
    ...
    接收三个参数：caseof代表调用角色数据（char）还是卡牌数据（card），mode设置调用模式（print打印、get获取全部、find搜索、new新增、del删除、edit修改），lookID根据模式不同填入不同参数。
    print：调用角色数据时，填写lookID可单独查看船员（c）、敌方（h）、其他（o）角色列表；调用卡牌数据时，填写lookID可单独查看攻击（a）、召唤（m）、支援（s）卡牌列表；返回全部角色/卡牌的字典。（新变动）
    get：返回全部角色的字典。（新变动）
    find：lookID为想要查找的角色/卡牌ID，返回角色/卡牌字典。
    new：lookID为想要新增的角色/卡牌字典，返回角色/卡牌字典。
    del：lookID为想要删除的角色/卡牌ID，返回角色/卡牌字典。
    edit：new + del，lookID同new。

    TODO: 修改modeGet()结构，获取打印分离。*done*
    '''
    fileDict = {}
    ...
    def modePrint(lookID): # 打印项目
        print('{:<5} {:<15} {:　<3} {:<6} {:<}'.format(header[0],header[1],header[2],header[3],header[4]))
        for k, v in fileDict.items():
            if lookID == groupList[0] and k[0] != groupList[0]:
                continue
            elif lookID == groupList[1] and k[0] != groupList[1]:
                continue
            elif lookID == groupList[2] and (k[0] == groupList[1] or k[0] == groupList[0]):
                continue
            ckID = eval(v)['id']
            ckName = eval(v)['name']
            ...
            try:
                if caseof == 'char':
                    ckRarity = '☆'*int(eval(eval(v)['info'])['rarity'])
                    ckSp = eval(eval(v)['info'])['class']
                elif caseof == 'card':
                    ckRarity = ['Bronze','Silver','Gold'][int(eval(eval(v)['info'])['rarity']) - 1]
...
def getSquad(caseof,mode='now',lookPreset=None):
    ...
    接收三个参数：caseof代表调用我方数据（ally）还是敌方数据（enemy），mode设置调用模式（now当前、get获取全部、find搜索、new新增、del删除、edit修改），lookPreset根据模式不同填入不同参数。
    now：查看当前我方/敌方编队。
    get：返回全部编队的字典。
    find：lookPreset为想要查找的编队名称，返回编队字典。
    new：lookPreset为想要新增的编队字典，返回编队字典。
    del：lookPreset为想要删除的编队名称，返回编队字典。
    edit：new + del，lookPreset同new。
