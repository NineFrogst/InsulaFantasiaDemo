...
def libCharacter():
    '''
    人物图鉴界面。
    可按照人物分组（cGroup）查看角色列表。

    通过把打印代码转移到DataIO.py内部，简化了此处代码。
    '''
    print("-----------\n人 物 图 鉴\n-----------\n1. 所有人物\n2. 船员图鉴\n3. 敌方图鉴\n4. 其他角色\n0. 返回前页")
    ...
    getCharacterDB('print',[None,'c','h','o'][eval(libChrSelect)-1])
    libChrAction()
    return
# 人物图鉴操作
def libChrAction():
    ...
    while True:
        ...
        if libChrAct == '1':
            ...
            cID = charDict['id']
            cName = charDict['name']
            cGroup = charDict['group']
            try:
                cRarity = '☆'*eval(eval(charDict['info'])['rarity'])
                cSp = eval(charDict['info'])['class']
            except:
                cRarity = 'null'
                cSp = 'null'
...
def libCard():
    ...
    getCardDB('print',[None,'a','s','m'][eval(libCrdSelect)-1])
    return libCrdAction()
# 卡牌图鉴操作
def libCrdAction():
    '''
